...
export default class FalsePositionMethod extends RootEquation{
    ...
    calculate(xl:number,xr:number,error:number,epsilon:number,equation:string) : object {
        ...
        let xm: number = (xl + xr) / 2; // Initial midpoint
        let prevXm: number = 0;
        let sign: number = this.function(xl, equation) * this.function(xm, equation); // Store the sign of f(xl) * f(xm)
        if (sign > 0) {
            xl = xm; // Move left endpoint to midpoint
        } else if (sign < 0) {
            xr = xm; // Move right endpoint to midpoint
        }
        error = Math.abs(xr - xl); // Calculate absolute error
        //first time
        this.listResult(listXl, xl);
        this.listResult(listXr, xr);
        this.listResult(listX1, xm);
        this.listResult(listError, error);
    
        //begin iteration
        while (error > epsilon && error !== Infinity && listError.length < 200) {
            prevXm = xm;
            xm = (xl + xr) / 2; // Calculate new midpoint
            sign = this.function(xl, equation) * this.function(xm, equation); // Store the sign of f(xl) * f(xm)
            ...
            error = Math.abs(xm - prevXm); // Calculate absolute error
            this.listResult(listError, error);
        }
